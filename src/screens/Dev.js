import React from 'react';
import {Link} from 'react-router-dom';

export default function Dev() {
    return (
        <div style={{display:'flex', flexFlow:'row wrap'}}>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app0">app0</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app1">app1</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app2">app2</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app3">app3</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app4">app4</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app5">app5</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app6">app6</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app7">app7</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app8">app8</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app9">app9</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app10">app10</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app11">app11</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app12">app12</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app13">app13</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app14">app14</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app15">app15</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app16">app16</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app17">app17</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app18">app18</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app19">app19</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app20">app20</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app21">app21</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app22">app22</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app23">app23</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app24">app24</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app25">app25</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app26">app26</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app27">app27</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app28">app28</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app29">app29</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app30">app30</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app31">app31</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app32">app32</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app33">app33</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app34">app34</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app35">app35</Link>
            </div>
            <div style={{width:'200px', height:'200px'}}>
                <Link to="/app36">app36</Link>
            </div>
        </div>
    );
}